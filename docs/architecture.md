...
| `@wdio/cdp-bridge` | Shared CDP bridge: single-target (`CdpBridge`) + multi-target (`MultiTargetCdpBridge`, one connection per window), with target classification injected per service |
