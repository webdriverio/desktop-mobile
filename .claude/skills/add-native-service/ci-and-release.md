...
- **`_ci-e2e-<framework>-ios.reusable.yml`** (iOS, `macos-latest`) — **two stages**, because a heavy `xcodebuild` on the same runner as the Appium session starves appium-xcuitest's SDK probe / drops `POST /session`:
  ...
  - **E2E job** (fresh runner): download + unpack the `.app` → re-scaffold the JS for Metro (no native build) → warm the Xcode toolchain (`xcrun --show-sdk-version`) → **resolve the sim device resiliently** (prefer the requested name, else the highest plain `iPhone <N>`; pin its UDID + propagate name/UDID via `$GITHUB_ENV` — never assume a name, since runner-image Xcode rolls drop old devices, #510/#511) → boot the sim → idempotent XCUITest install → **pre-build WebDriverAgent into an explicit `RN_WDA_DD` derivedDataPath** (the conf reads it via `appium:derivedDataPath` + `usePrebuiltWDA`) → Metro start + **pre-bundle** (`curl …/index.bundle`, avoids the "Bundling…" overlay race) → specs.
