...
- **Auto binary-path detection** — the *mechanism* is framework-specific (Electron reads Forge/Builder config), but the *user-facing option* (`appBinaryPath`/`application`) is standard. Port auto-detection only when the framework has one canonical build tool with a stable output layout; otherwise expose the standard explicit-path option and skip the detection machinery (guessing wrong is worse than asking). The *API stays consistent* either way. Both Dioxus and Tauri (since v1.x) trust the user-supplied path — Tauri's earlier resolver baked in the v1 single-crate `<appPath>/src-tauri/target/debug/<productName>` layout and broke for Cargo workspaces, release builds, and `productName`-vs-Cargo-package-name mismatches (issue #295, fix #303). It's the canonical case study for why "convenience" auto-detection is brittle.
