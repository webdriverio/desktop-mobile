...
CI-gated).** This is the mobile reality behind #445/#446/#457 (gotcha 19 +
[plumbing-mobile.md](plumbing-mobile.md) → "Known first-ship gaps"), and it also hit **desktop**:
...
19. **(Mobile) a *mechanism-only* standard feature inherits into the next mobile service silently.** Convergence says ship every standard feature at full parity, but RN/Flutter shipped three of them **mechanism-first, validation-deferred** — and deferred by **mobile CI cost/fragility, not an upstream block** (so this is *not* the [upstream-blocks](#when-upstream-blocks-the-standard-surface-shipping-pre-10) case): **multiremote** is capability-shape-parsed only, one udid across all instances, no worker `isMultiremote` branch (#446); **standalone/session** ships helpers + unit tests but **no E2E** and two undocumented divergences — external Appium, no device claiming (#445); **deeplink** is implemented + unit-tested but **smoke-only** — no nav assertion, no fixture scheme, not CI-gated (#457). "The mechanism works" (unit-tested) ≠ "the feature is proven" (E2E-asserted + CI-gated) ≠ "the README may claim it". A second mobile service that clones the RN template inherits all three **silently** — the exact trap as gotcha 18. **Default to finishing each in the main cycle** — of the three, only real multiremote *earned* its deferral (large + needs a second device); deeplink and standalone-docs were closer to *slips* (cheap validation that piggybacks the already-booted emulator, plus a README that overclaimed "✅ tested"). Reserve "defer behind your own tracked issue + a README/`ROADMAP.md` note" for the genuinely large/infra-heavy case, and make the version tell the truth (a Tier-1 gap at 1.0 overclaims → prefer `0.x`). Detail + the per-gap verdicts → [plumbing-mobile.md](plumbing-mobile.md) → "Known first-ship gaps".
