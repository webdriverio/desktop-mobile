...
21. **Standalone teardown: rethrow via `failStartup`, don't hand-roll it.** On a startup failure, `session.ts`'s `init()` runs best-effort teardown then rethrows the original error. Use `@wdio/native-utils`' `failStartup(startupError, label, ...teardowns)` with your own teardown thunks — they let each service compose its own non-uniform steps (e.g. Electron also closing its log writer) and keep service types out of the helper. A teardown failure joins the original in an `AggregateError`, so pass only steps whose failure means a **leak the user must act on** (launcher `onComplete` → orphaned driver/dev server/Metro, log-writer close). Delete the session with `safeDeleteSession` *before* `failStartup`, not as a thunk: after a startup failure a delete error is almost always a symptom of the same fault (`chrome not reachable`, a dead driver), and aggregating it would swap the real error's headline for the generic `<label> startup and cleanup failed` — while leaking nothing that `onComplete`/process exit doesn't reap. Never a bespoke swallow-and-warn (the old Dioxus shape hid teardown failures) or a copy of a sibling's block (they drift). Bound `onComplete` with `boundedOnComplete`; if it stops child processes (drivers, Metro), pass `timeoutMs: PROCESS_TEARDOWN_TIMEOUT_MS` — the default deadline equals a single process's SIGTERM grace, so it would abandon the stop mid-escalation. `cleanup()` of an *already-established* session still swallows — a successful session's teardown shouldn't throw.
