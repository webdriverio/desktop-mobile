...
20. **(Mobile/iOS CI) never hardcode the simulator device name — resolve it resiliently.** GitHub runner images roll their Xcode forward, and each Xcode ships a *different* default simulator set, so a leg pinned to an exact name (e.g. `appium:deviceName: 'iPhone 16'`) fails **deterministically** once the image drops that device — it only "passes on re-run" while older images linger in the pool, then hard-fails everywhere (#510). At boot, resolve a concrete device: prefer the requested name, else **fall back to the newest available `iPhone <N>`** (parse the number, newest runtime), **pin the resolved UDID** (this also removes the duplicate-name boot ambiguity, #359), and log the fallback. Don't "fix" a drift break by bumping the hardcoded name to a currently-present device — that just re-arms the trap for the next image roll. The resilient resolver is a **shared mobile-CI concern** (every mobile service's iOS leg needs it), so it belongs in the shared layer, not copy-pasted per service.
