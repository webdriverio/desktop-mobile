...
21. **Standalone teardown: rethrow via `failStartup`, don't hand-roll it.** On a startup failure, `session.ts`'s `init()` runs best-effort teardown (launcher `onComplete`, `deleteSession`, log-writer close — whatever it set up) then rethrows the original error, surfacing any teardown failure beside it in an `AggregateError` instead of masking it. Use `@wdio/native-utils`' `failStartup(startupError, label, ...teardowns)` with your own teardown thunks — they let each service compose its own non-uniform steps (e.g. Electron also closing its log writer) and keep service types out of the helper. Never a bespoke swallow-and-warn (the old Dioxus shape hid teardown failures) or a copy of a sibling's block (they drift). `cleanup()` of an *already-established* session still swallows — a successful session's teardown shouldn't throw.
